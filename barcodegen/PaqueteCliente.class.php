...
class PaqueteCliente {
    ...
    function __construct() {
        ...
        $ms = new MS();
        $ms->Query("select Address,City,Country,Phone1,LicTradNum,CardName,AliasName  from OCRD  WHERE CardCode ='$cod_cli'");
        $ms->NextRecord();
        $DirCli = $ms->Record['Address']; 
        $CiudadCli = $ms->Record['City']; 
        $PaisCli = $ms->Record['Country']; 
        $PhoneCli = $ms->Record['Phone1']; 
        $ruc_cli = $ms->Record['LicTradNum'];
        $cliente = $ms->Record['CardName'];
        $Alias = $ms->Record['AliasName'];
