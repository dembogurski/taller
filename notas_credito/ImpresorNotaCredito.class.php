...
class ImpresorNotaCredito {
   function __construct() {
        ...
        $sql_cli ="SELECT  DATE_FORMAT(fecha,'%d de %M de %Y') AS fecha,cliente  FROM nota_credito WHERE estado = 'Cerrada' AND n_nro = $nro_nota";
        $db->Query($sql_cli);
        ...
        $sql_det = "select lote as codigo,descrip,cantidad,um_prod, precio_unit,subtotal from nota_credito_det where n_nro = $nro_nota";
        $db->Query($sql_det);
        ...
        $db->Query("update nota_credito set fact_nro = $nota_credito_contable,pdv_cod = '$pdv',tipo_fact = '$man_pre',tipo_doc = '$tipo_doc'  where n_nro = $nro_nota");
