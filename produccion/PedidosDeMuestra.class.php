...
function buscarCliente(){
    ...
    $ms = new MS();
    $sql = "select top 50 CardCode,CardName from OCRD o WHERE o.CardName like '%$criterio%' or LicTradNum like '$criterio%'";
...
function buscarArticulo(){
    $criterio = $_REQUEST['criterio'];
    $ms = new MS();
    $sql = "select ItemCode,ItemName from oitm where (ItemCode like '$criterio%' or ItemName like '%$criterio%') and ItemName not like 'ACTIVOS%'";
    echo json_encode(getResultArrayMSSQL($sql));
}
...
function getUbic(){
    ...
    $lote = $_REQUEST['lote'];
    $suc = $_REQUEST['$suc'];
    $ub = "select CONCAT(U_nombre,'-',U_fila,'-',U_col) as U_ubic, CONCAT(U_nombre,U_col) as Nodo  from [@REG_UBIC] where  U_codigo = '$codigo' and U_lote = '$lote' and U_suc = '$suc'";
    echo json_encode(getResultArrayMSSQL($ub));
}
...
function insertarEnSolicitudTraslado() {
    ...
    $ms = getResultArrayMSSQL("select o.ItemCode, i.AvgPrice as PrecioCosto,i.ItemName,i.InvntryUom as um_prod from OBTN o,OITM i WHERE  o.ItemCode = i.ItemCode and  o.DistNumber = '$lote';");
...
function procesamientoColores(){
    $usuario = $_POST['usuario'];
    $suc = $_POST['suc'];
    
    $sql ="SELECT m.nro,cliente,codigo,articulo, medida,m.cantidad,lote,color, img ,d.estado FROM catalogo_muestras m, det_catalogo d  WHERE m.nro = d.nro AND (d.usuario = '$usuario' OR encargado = '$usuario') AND m.estado <> 'Cerrado' ORDER BY m.nro ASC ";
    $db = new My();
    $ms = new MS();
    $db->Query($sql);
    ...
    $old_nro = '';
    while($db->NextRecord()){
        if($nro != $old_nro && $old_nro != ''){
           $t->Show("catalogos_en_proceso_foot");
        }  
        $nro = $db->Record['nro'];
        ...
        if($nro != $old_nro){
           $t->Show("catalogos_en_proceso_head");
        }        
        $lote = $db->Record['lote'];
        ...
        $qstock = "select Quantity from oibt where ItemCode ='$codigo' and BatchNum = '$lote' and WhsCode = '$suc'";
        $ms->Query($qstock);
        $ms->NextRecord();
        ...
        $t->Set("img",$img);
        $t->Set("estado",$estado);
        $t->Show("catalogos_en_proceso_body");
...
function imprimirContratapa(){
    ...
    $datos =   getResultArrayMSSQL("select   U_COMPOSICION as composicion,U_GRAMAJE_PROM as gramaje ,U_ANCHO as ancho  from oitm where ItemCode = '$codigo'");
    ...
    $ms = new MS();
    $ms->Query("select   CONVERT(DECIMAL(10,0), ROUND(Quantity, 1, 1))  as Stock ,count(*) as Rollos from oibt where ItemCode ='$codigo'  and WhsCode = '00'
    AND Quantity > 0 and U_padre  = '' group by   CONVERT(DECIMAL(10,0), ROUND(Quantity, 1, 1))   order by count(*) desc , Stock   desc");
    $rolls = "";
    $roll_title= "";
    while($ms->NextRecord()){
    ...
    $ms->Query("select   CONVERT(DECIMAL(10,0), ROUND(Quantity, 1, 1))  as Stock ,count(*) as Piezas from oibt where ItemCode = '$codigo'  and WhsCode = '00'
    AND Quantity > 0 and U_padre  != '' group by   CONVERT(DECIMAL(10,0), ROUND(Quantity, 1, 1))   order by count(*) desc , Stock   desc");
    $pz = "";
    $pz_title = "";
    while($ms->NextRecord()){
