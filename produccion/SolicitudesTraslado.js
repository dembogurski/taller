...
function verSolicitudes(mobile,tactil){
    ...
    if(!mobile){
        var url = "produccion/ReporteSolicitudes.class.php?origen="+origen+"&destino="+destino+"&usuario="+usuario+"&desde="+desde+"&hasta="+hasta+"&paper_size="+paper_size+"&tipo="+tipo+"&urge="+urge+"&estado="+estado+"&tipo_filtro="+tipo_filtro;
        if(tactil){
            url = "produccion/ReporteSolicitudesTactil.class.php?origen="+origen+"&destino="+destino+"&usuario="+usuario+"&desde="+desde+"&hasta="+hasta+"&paper_size="+paper_size+"&tipo="+tipo+"&urge="+urge+"&estado="+estado+"&nivel="+nivel+"&tipo_filtro="+tipo_filtro;;
        }
...
function getPedidosFiltrados(){
    ...
    $.ajax({
        ...
        data: {"action": "getPedidosFiltrados", desde: desde,hasta:hasta,nivel:nivel,destino:destino,tipo_busqueda:tipo_busqueda},
        async: true,
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (data) { 
            ...
            for (var i in data) { 
                ...
                $("#origen").append("<option value='"+suc+"'>"+suc+"  -  "+nombre+"    "+items+"   "+nom+"    ["+fecha+"]</option>");
            }  
            $("#msg").html(""); 
            if(!set_ant){  
...
function getPedidos(origen,destino,desde,hasta,hora,nivel,mayorista,urge,estado){
     ids = new Array();
     $("#filters").fadeOut();
     $(".rotate").fadeOut();
     $.ajax({
        type: "POST",
        url: "produccion/SolicitudesTraslado.class.php",
        data: {"action": "getPedidos",origen:origen,destino:destino,desde:desde,hasta:hasta,hora:hora,nivel:nivel,mayorista:mayorista,urge:urge,estado:estado},
        async: true,
        dataType: "json",
        beforeSend: function () {
            $("#camino").html("<img src='img/loading_fast.gif' width='16px' height='16px' >"); 
        },
        success: function (data) {
            pedidos = 0;
            for (var i in data) { 
                ...
                var pedido = {nodo:nodo, prioridad:prioridad, ubic:ubic,nro:nro,codigo:codigo,lote:lote,lote_rem:lote_rem,descrip:descrip,cantidad:cantidad,mayorista:mayorista,urge:urge,estado:estado,obs:obs,usuario:usuario};
                ids[id] = pedido;
                pedidos++;
            }  
            pendientes = pedidos;
            $("#pend").html(pendientes);  
            $("#camino").html(""); 
            cursor(0);
            $("#nodos").fadeIn();
        }
