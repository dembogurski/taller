...
// Metodo para cargar una factura Abierta de Usuario
/*
function cargarFactura(factura){
    var usuario = getNick(); 
    var session = getCookie(usuario).sesion;    
    load("ventas/CargarVenta.class.php",{usuario:usuario,session:session,factura:factura,suc:getSuc()}, function(){    
...
function cargarNotaCredito(nro_nota,factura,moneda){
    var usuario = getNick(); 
    var session = getCookie(usuario).sesion;    
    load("notas_credito/CargarNotaCredito.class.php",{usuario:usuario,session:session,nro_nota:nro_nota,factura:factura}, function(){ $("#area_carga").fadeIn("slow",function(){ buscarDetallesFactura(factura,nro_nota,false);flag = true;  });   } );
}
