...
function verExtracto() {
    ...
    while ($db->NextRecord()) {
        ...
        if($Cuota == 1){        
            $pagos_inmediatos = "SELECT SUM( IF(entrada-salida IS NULL, 0, entrada-salida )) + SUM( IF(monto IS NULL,0, monto)) + SUM( IF(t.valor IS NULL,0, t.valor)) AS entrega_inmediata
            FROM factura_venta p LEFT JOIN efectivo e ON e.f_nro = p.f_nro    LEFT JOIN convenios c ON c.f_nro = p.f_nro    LEFT JOIN cheques_ter t ON t.f_nro = p.f_nro WHERE p.f_nro =  $TicketInterno;";
        ...
        $pagos_parciales = "SELECT  '*' AS id_pago,p.suc, DATE_FORMAT(p.fecha,'%d-%m-%Y') AS fecha,p.nro_recibo AS recibo, SUM(entrada-salida)  AS efectivo,  SUM(monto) AS tarjetas, SUM(t.valor) AS cheques,  
        SUM( IF(entrada-salida IS NULL, 0, entrada-salida )) + SUM( IF(monto IS NULL,0, monto)) + SUM( IF(t.valor IS NULL,0, t.valor)) AS entrega_actual,'Si' as control_caja
        FROM factura_venta p  
        LEFT JOIN efectivo e ON e.f_nro = p.f_nro 
        LEFT JOIN convenios c ON c.f_nro = p.f_nro 
        LEFT JOIN cheques_ter t ON t.f_nro = p.f_nro WHERE p.f_nro = $TicketInterno;";
        
        $pagos_recibidos = "SELECT p.id_pago,p.suc, DATE_FORMAT(p.fecha,'%d-%m-%Y') AS fecha,p.folio_num AS recibo, (SELECT SUM(entrada-salida) FROM efectivo e WHERE e.trans_num = p.id_pago)  AS efectivo,  SUM(monto) AS tarjetas, SUM(t.valor) AS cheques,entrega_actual,control_caja
        ...
        $det->Query("$pagos_recibidos  union $pagos_parciales");
        
         
        
        if($det->NumRows() > 1 || $entrega_inmediata > 0  ){
            $t->Set("visible", "");            
        }else{
        ...
        while ($det->NextRecord()) {
            ...
            $ControlCaja = $det->Record['control_caja'];
            ...
            $t->Set("verificado", "No_verificado");  
            if($ControlCaja == "Si"){
                $t->Set("verificado", "Si_verificado");  
            } 
