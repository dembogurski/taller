...
function checkRUC(Obj) {
    ...
    if (!validRUC(ruc) && tipo_doc == "C.I." && !isNaN(ruc)) {
        $.ajax({
            type: "POST",
            url: "Ajax.class.php",
            data: { "action": "calcularDV", "ci": parseInt(ruc) },
            async: true,
            dataType: "json",
            beforeSend: function() {
            ...
            if (data.length > 0) {
                if (data.length > 1) {
                    ...
                    for (var i in data) {
                        ...
                        $("#lista_clientes").append("<tr class='tr_cli_data fila_cliente_" + i + "' data-tipo_doc='" + TipoDoc + "'><td class='item'><span class='codigo' data-moneda='" + moneda + "'>" + codigo + "</span></td>   <td class='item'><span class='ruc'>" + ruc + "</span> </td><td  class='item'><span class='cliente'>" + cliente + "</span></td><td>"+telefonos+"</td><td class='itemc'><span class='cat'>" + cat + "</span></td></tr>");
                    }
                    cant_clientes = cont;
