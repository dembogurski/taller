...
function configurar(){
   ...
   $(".falla").change(function(){
        var stock = parseFloat($("#metros").val().replace(",","."));
...
function showKeyPadFallas(id){
    ...
    showNumpad(id,calcFallas,false);     
    var position = $("#"+id).offset();     
   //console.log('top'  + position.top  +'left' + position.left);
    $('#n_keypad').css({ 'top'  : position.top -56 ,'left' : position.left -7});
...
function calcularGramaje(){
    ...
    var gramaje_ref = parseFloat($("#gramaje_ref").val());
        ...
        success: function(data){ 
            ...
            if( existe === "true" ){
                ...
                $(".suc_pol").each(function(){
                    var suc = $(this).text();    
                    buscarPoliticaCodigoXSuc(codigo,suc);         
                });
                getFallas(lote);
...
function getMetradorData(ip_domain,port,intentos,destino){  
    $("#"+destino).val(""); 
    if(intentos < 4){
...
function registrarFalla(Fx){
    ...
    var padre = $("#lote").val();
    var codigo = $("#codigo").val();
    //console.log(tipo_falla);
    $.ajax({
        type: "POST",
        url: "compras/Fraccionar.class.php",
        data: {action: "registrarFalla", suc: getSuc(),codigo:codigo,padre:padre,usuario: getNick(),tipo_falla:tipo_falla,mts:mts},
        async: true,
        dataType: "json",
        beforeSend: function () {
    ...
    $.ajax({
        ...
        success: function (data) {   
            for(var i in data){
                ...
                var mts = data[i].mts;
                $("#fallas").append("<tr class='row_tipo_falla'><td class='itemc'>"+tipo_falla+"</td><td class='itemc'>"+mts+"</td></tr>");
            }
            $("#msg").html("");
        },
        error: function (e) {                 
    ...
    $.ajax({
        ...
        success: function (data) {   
            ...
            if(data.length > 0){
                ...
                st_comp+="<tr><th colspan='6' style='background:orange;'>Stock Comprometido</th><th style='text-align:center;background:white'>X</th></tr>";
