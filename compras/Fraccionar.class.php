...
function getRemisionAbierta($suc,$suc_d,$usuario){
    $db = new My();
    $db->Query("SELECT n_nro FROM nota_remision WHERE estado = 'Abierta' AND suc = '$suc' AND suc_d = '$suc_d' AND usuario IN('derlisg','TonyG') ");
    if($db->NumRows() > 0){
        $db->NextRecord();
        $nro = $db->Record['n_nro'];
        return $nro;
    }else{
       $nro =  generarRemito($suc,$suc_d,'derlisg');
       return $nro;
    }
}
