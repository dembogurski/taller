...
class Invoice {
    function __construct() {         
        ...
        $ms = new MS();
        $ms->Query("select Street,City,UPPER(p.Name) as Country  from CRD1 d,OCRY p WHERE d.Country = p.Code and  CardCode = '$cod_prov'");
        $Address = "";
        $City = "City";
        $Country = "Country";
        if($ms->NumRows()>0){
            $ms->NextRecord();
            $Street = $ms->Record['Street'];
            $City = $ms->Record['City'];
            $Country = $ms->Record['Country'];
            $Address = $Street.", ".$City." ".$Country;
        } 
        $ms->Query("select Phone1,E_Mail from ocrd where CardCode =  '$cod_prov'");
        $Phone = "";
        $E_Mail = "";
        if($ms->NumRows()>0){
            $ms->NextRecord();
