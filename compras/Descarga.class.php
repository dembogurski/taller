...
function fraccionar(){
    ...
    $db->Query("update entrada_det set cantidad = cantidad-$suma,cant_calc =cantidad-$suma, /*precio = $precio ,*/ subtotal = precio * cantidad where id_ent = $id_ent and id_det = $id_det");
...
function controlarEntradaMercaderiasNoDescargadas() {
    ...
    COUNT(CONCAT( s.codigo , s.lote)) AS en_stock   FROM entrada_det d LEFT JOIN stock s ON d.codigo = s.codigo AND d.lote = s.lote AND d.id_ent = s.nro_identif AND s.tipo_ent ='EM' WHERE d.id_ent = $nro_compra;";
    echo json_encode(getResultArray($sql));
}
...
function clasificarPiezas(){
    ...
    $sql = "SELECT DISTINCT lote,presentacion,suc FROM orden_procesamiento WHERE lote IN($lotes)";
