...
class SubirPedidoExel{
    ...
    private function getColorList(){
        $link = new Ms();
        $colores = array();
        $query = "select Code,Name from [@EXX_COLOR_COMERCIAL] where U_estado='Activo'";
        $link->Query($query);
        
        while($link->NextRecord()){
            $colores[utf8_encode($link->Record['Code'])] = utf8_encode($link->Record['Name']);
    ...
    public function getColor($color,$json = false){
        $link = new Ms();
        $link->Query("select Code,Name from [@EXX_COLOR_COMERCIAL] where U_estado='Activo' and Name = '$color'");
        if( $link->NumRows() > 0 ){
            $link->NextRecord();
            $data = $link->Record; 
            $link->Close();
    ...
    public function getItemData($itemCode,$json = false){
        $link = new Ms();
        $link->Query("select ItemCode,ItemName,InvntryUom from OITM where ItemCode ='$itemCode'");
        if( $link->NumRows() > 0 ){
            $link->NextRecord();
            $data = $link->Record; 
            $link->Close();
