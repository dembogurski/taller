...
class Arqueo{ 
    ...
    function __construct() {
		...
		$this->template->Set('user', $_REQUEST['usuario']);
		...
		if($usuarios){
			...
			foreach ($usuarios as $usu){				
				...
				$this->template->Set('convenio',$this->moneda($usu['credito']['Asociacion'],0));
				...
				$total = $usu['efectivo']+$usu['credito']['Tarjeta Credito']+$usu['credito']['Tarjeta Debito']+$usu['credito']['Asociacion']+$usu['credito']['Criptomoneda']+$usu['cheques']['Al Dia']+$usu['cheques']['Diferido']+$usu['cuotas']/*+$usu['reservas']*/-$usu['devoluciones'];				
				
				$t_contado +=$usu['efectivo'];
				$t_credito +=$usu['credito']['Tarjeta Credito'];
				$t_debito +=$usu['credito']['Tarjeta Debito'];
				$t_convenio +=$usu['credito']['Asociacion'];
		...
		$this->template->Set('t_convenio',$this->moneda($t_convenio,0));
		...
		if(count($cheques_v)>0){
			$this->template->Set('ex_head',"<th colspan='7' >Cheques Registrados por Ventas</th>");
			$this->template->Show("extra_head");
			$this->template->Set('ex_sub_head',"<th>Cliente</th><th>Banco</th><th>Cuenta</th><th>N\272</th><th>Valor</th><th>Fecha Emision</th><th>Fecha Pago</th>");
			...
			foreach($cheques_v as $cheque){
				...
				$this->template->Set('ex_data',"<td class='item'>".utf8_decode($cheque['benef'])."</td><td class='item'>".utf8_decode($cheque['nombre'])."</td><td class='num'>".$cheque['cuenta']."</td><td class='num'>".$cheque['nro_cheque']."</td><td class='num'>".$this->moneda($cheque['valor_ref'],2)."</td><td class='item'>".$cheque['emis']."</td><td class='item'>".$cheque['pago']."</td>");
		...
		if(count($cheques_c)>0){
			$this->template->Set('ex_head',"<th colspan='7'  >Cheques Registrados por Cobro de Cuotas</th>");
			$this->template->Show("extra_head");
			$this->template->Set('ex_sub_head',"<th>Cliente</th><th>Banco</th><th>Cuenta</th><th>N\272</th><th>Valor</th><th>Fecha Emision</th><th>Fecha Pago</th>");
			$this->template->Set('ex_sub_head_conf','class="report_header"');
			$this->template->Show("extra_sub_head"); 			
			...
			foreach($cheques_c as $cheque){
				$t_total += $cheque['valor_ref'];
				$this->template->Set('ex_data',"<td class='item'>".$cheque['benef']."</td><td class='item'>".$cheque['nombre']."</td><td class='num'>".$cheque['cuenta']."</td><td class='num'>".$cheque['nro_cheque']."</td><td class='num'>".$this->moneda($cheque['valor_ref'],2)."</td><td class='itemc'>".$cheque['emis']."</td><td class='itemc'>".$cheque['pago']."</td>");
				$this->template->Show("extra_body"); 
			}
			$this->template->Set('ex_foot',"<td colspan='4' class='item'>Total: </td><td class='num'>".$this->moneda($t_total,2)."</td><td colspan='2'></td>");
			$this->template->Show("extra_foot"); 
		...
		$this->template->Show("extra_head"); 
		$this->template->Set('ex_sub_head',"<th>Nro.</th><th>Vendedor</th><th>Cliente</th><th>Factura N\272</th><th>Moneda</th><th>Total</th>");
		$this->template->Show("extra_sub_head"); 
		$t_total = 0;
		...
		$this->template->Set('ex_head',"<th colspan='7'><span id='cab_cm_div' data-target='det_cm_div' onclick='toggle($(this))' class='hide'>Cambio de Divisas</span></th>");
		$this->template->Show("extra_head"); 
		...
		if(count($cobros)>0){
			...
			$this->template->Set('cobros_ex_head',"<th colspan='2' $verif_cobros >Cobros</th>");
			$this->template->Show("cobros_head"); 
			$this->template->Set('ex_sub_head',"<th>Metodo</th><th>Total</th>");
			$this->template->Set('ex_sub_head_conf',"");			
			$this->template->Show("ex_sub_head"); 
			$t_total = 0;
	...
	private function getUsersMovs($suc,$desde,$hasta){
		$data = array();
		$link = new My();
		$link->Query("SELECT u.usuario,concat(u.nombre,', ',u.apellido) as vendedor from factura_venta f inner join usuarios u using(usuario) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' union select u.usuario,concat(u.nombre,', ',u.apellido) as vendedor from reservas r inner join usuarios u using(usuario) where r.suc='$suc' and r.fecha between '$desde' and '$hasta' /*and r.estado = 'Pendiente'*/ union select u.usuario,concat(u.nombre,', ',u.apellido) as vendedor from nota_credito n inner join usuarios u using(usuario) where n.suc='$suc' and n.fecha between '$desde' and '$hasta' and n.estado = 'Cerrada' group by usuario");
	...
	private function getVentasConvenio($user,$suc,$desde,$hasta){
		...
		$link->Query("SELECT c.cod_conv,c.monto from convenios c inner join factura_venta f using(f_nro) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and f.usuario = '$user'");
		while($link->NextRecord()){
			$cod_conv = $link->Record['cod_conv'];
			$data[$convenios[$cod_conv]] += (float)$link->Record['monto'];
		}
		$link->Close();
		return $data;
	}
	...
	private function getCheques($suc,$desde,$hasta,$tipo){
		$link = new My();
		if($tipo == "ventas"){
			$query = "SELECT benef,b.nombre,c.cuenta,c.nro_cheque,c.valor_ref,DATE_FORMAT(c.fecha_emis,'%d-%m-%Y') as emis,DATE_FORMAT(c.fecha_pago,'%d-%m-%Y') as pago from cheques_ter c inner join bancos b using(id_banco) where c.id_concepto=1 and c.fecha_ins between '$desde' and '$hasta' and c.suc = '$suc' and estado <> 'Anulado'";
		}else{
			$query = "SELECT benef,b.nombre,c.cuenta,c.nro_cheque,c.valor_ref,DATE_FORMAT(c.fecha_emis,'%d-%m-%Y') as emis,DATE_FORMAT(c.fecha_pago,'%d-%m-%Y') as pago from cheques_ter c inner join bancos b using(id_banco) where c.id_concepto=8 and c.fecha_ins between '$desde' and '$hasta' and c.suc = '$suc' and estado <> 'Anulado'";
		}
	...
	private function getCobros($suc,$desde,$hasta){
		...
		$convenios = "SELECT 'Convenio',pagos.moneda,pagos.id_pago, sum(pagos.valor), sum(c.monto) as mov from (SELECT p.moneda,p.id_pago,d.valor,p.suc,p.fecha from pagos_recibidos p inner join pago_rec_det d  using(id_pago) where p.suc='$suc' and p.fecha between '$desde' and '$hasta' $group_by) as pagos inner join convenios c on pagos.id_pago = c.trans_num";
