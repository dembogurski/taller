...
class Arqueo{ 
    ...
    function __construct() {
		...
		$usuarios = $this->getUsersMovs($this->suc,$this->desde,$this->hasta,'Articulo'); // Por Articulo
		$t_contado=0;
		$t_credito=0;
		$t_debito=0;
		$t_convenio=0;
		...
		if($usuarios){
                    ...
                    foreach ($usuarios as $usu){	 			
                            ...
                            $total = $usu['efectivo']+$usu['credito']['Tarjeta Credito']+$usu['credito']['Tarjeta Debito']+$usu['credito']['Retencion']+$usu['credito']['Criptomoneda']+$usu['credito']['Pago QR']+$usu['cheques']['Al Dia']+$usu['cheques']['Diferido']+$usu['cuotas']/*+$usu['reservas']*/-$usu['devoluciones'];				
		...
		$this->template->Show("foot");
                ...
                $usuarios = $this->getUsersMovsServicio($this->suc,$this->desde,$this->hasta,'Servicio'); // Por Servicio
		$t_contado=0;
                $t_cuotas =0;
                $t_cheques =0;
		$t_total=0;
                ...
                if($this->usuario == "douglas"){
                   // print_r($usuarios);
                } 
                
		if($usuarios){ 
                    $this->template->Show("head_servicio");        
                    foreach ($usuarios as $usu){	 			
                            $total=0;

                            $this->template->Set('usuario',$usu['usuario']);
                            $this->template->Set('vendedor',$usu['vendedor']);
                            $this->template->Set('cliente',$usu['cliente']);
                            $this->template->Set('detalle',$usu['descrip']);
                            $this->template->Set('contado',$this->moneda($usu['efectivo'],0));                            
                            $this->template->Set('cuotas',$this->moneda($usu['cuotas'],0));
                            $this->template->Set('cheques',$this->moneda($usu['cheques'],0));

                            $total = $usu['efectivo'] + $usu['cuotas'] + $usu['cheques']  ;				

                            $t_contado +=$usu['efectivo'];                            
                            $t_cuotas +=$usu['cuotas'];
                            $t_cheques +=$usu['cheques'];
                            $t_total +=$total;
                            $this->template->Set('total',$this->moneda($total,0));
                            if((double)$total!==0.00){
                                $this->template->Show("data_servicio"); 
                            }
                    }	
                    $this->template->Set('t_contado',$this->moneda($t_contado,0));                                
                    $this->template->Set('t_cuotas',$this->moneda($t_cuotas,0));		
                    $this->template->Set('t_cheques',$this->moneda($t_cheques,0));		        
                    $this->template->Set('t_total',$this->moneda($t_total,0));
                    $this->template->Show("total_servicio");
                    $this->template->Show("foot");    
		}
                            
                
                //####################################### Fin Facturas x Servicio ########################################## 
		...
		if(count($cheques_v)>0){
			...
			$this->template->Show("extra_foot"); 
                ...
                if(count($cuotas)>0){
                    $this->template->Set('ex_head',"<th colspan='6' >Ventas a Credito</th>");
                    $this->template->Show("extra_head");
                    $this->template->Set('ex_sub_head',"<th style='width:10%'>Ticket</th><th style='width:5%'>Moneda</th><th style='width:40%'>Cliente</th><th style='width:5%'>Cuotas</th><th style='width:10%'>Total Cuotas</th> ");
                    $this->template->Show("extra_sub_head"); 
                    $t_total = 0;
                    $t_total_srv = 0;
                    foreach($cuotas as $cuota){
                        ...
                        if($clase == "Articulo"){
                            ...
                            $total_srv = $cuota['total'];
                            $total = 0;
                            $t_total_srv += $cuota['total'];
                        }
                            
                        $this->template->Set('ex_data',"<td class='item ".$clase."'>".$cuota['f_nro']."</td><td class='itemc'>".$cuota['moneda']."</td><td class='item'>".$cuota['cliente']."</td><td class='itemc'>".$cuota['cuotas']."</td><td class='num'>".$this->moneda($total,2)."</td>");
                        $this->template->Show("extra_body"); 
                    }
		...
		if(count($cheques_c)>0){
			...
			foreach($cheques_c as $cheque){
				$t_total += $cheque['valor_ref'];
				$this->template->Set('ex_data',"<td class='item' title='ID Cobro: ".$cheque['id_cobro']."'>".$cheque['benef']."</td><td class='item'>".$cheque['nombre']."</td><td class='num'>".$cheque['cuenta']."</td><td class='num'>".$cheque['nro_cheque']."</td><td class='num'>".$this->moneda($cheque['valor_ref'],2)."</td><td class='itemc'>".$cheque['emis']."</td><td class='itemc'>".$cheque['pago']."</td><td class='itemc'>".$cheque['tipo']."</td>");
				$this->template->Show("extra_body"); 
			}
			$this->template->Set('ex_foot',"<td colspan='4' class='item'>Total: </td><td class='num'>".$this->moneda($t_total,2)."</td><td colspan='2'></td>");
			$this->template->Show("extra_foot"); 
                ...
                if(count($cobros)>0){
                    ...
                    foreach($cobros as $cobro){
                        $t_total += $cobro['valor'];
                        $this->template->Set('ex_data',"<td class='item' title='ID Cobro: ".$cobro['id_cobro']."'>".$cobro['cliente']."</td><td class='num'>".$this->moneda($cobro['valor'],2)."</td><td class='itemc'>".$cobro['recibo']."</td>");
                        $this->template->Show("extra_body"); 
                    }
	...
	private function getUsersMovs($suc,$desde,$hasta,$clase){
		$data = array();
		$link = new My();
		$link->Query("SELECT u.usuario,concat(u.nombre,', ',u.apellido) as vendedor from factura_venta f inner join usuarios u using(usuario) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and clase = '$clase' union select u.usuario,concat(u.nombre,', ',u.apellido) as vendedor from reservas r inner join usuarios u using(usuario) where r.suc='$suc' and r.fecha between '$desde' and '$hasta'   union select u.usuario,concat(u.nombre,', ',u.apellido) as vendedor from nota_credito n inner join usuarios u using(usuario) where n.suc='$suc' and n.fecha between '$desde' and '$hasta' and n.estado = 'Cerrada' group by usuario");
		while($link->NextRecord()){
			$user = $link->Record['usuario'];
			array_push($data,[
				"usuario"=>$user,
				"vendedor"=>$link->Record['vendedor'],
				"efectivo"=>$this->getVentasContado($user,$suc,$desde,$hasta,$clase),
				"credito"=>$this->getVentasConvenio($user,$suc,$desde,$hasta,$clase),
				"cheques"=>$this->getVentasCheque($user,$suc,$desde,$hasta,$clase),
				"cuotas"=>$this->getVentasCuotas($user,$suc,$desde,$hasta,$clase),
        ...
        private function getUsersMovsServicio($suc,$desde,$hasta,$clase){
		$data = array();
		$link = new My();
		$link->Query("SELECT f.f_nro, u.usuario,concat(u.nombre,', ',u.apellido) as vendedor, f.cliente, GROUP_CONCAT(d.descrip) AS descrip  FROM factura_venta f INNER JOIN fact_vent_det d  USING (f_nro)  inner join usuarios u using(usuario) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and clase = '$clase' group by  usuario,cliente,f.f_nro");
		while($link->NextRecord()){
			$user = $link->Record['usuario'];
			array_push($data,[
				"usuario"=>$user,
				...
				"efectivo"=>$this->getVentasContadoServicio($link->Record['f_nro']),				
				"cuotas"=>$this->getVentasCuotasServicio($link->Record['f_nro']),
				"cheques"=>$this->getChequesServicio($link->Record['f_nro'])  
			]);
		}
	...
	// Ventas contado
	private function getVentasContado($user,$suc,$desde,$hasta,$clase){
		$data = array();
		$link = new My();
		$monto = 0;
		$link->Query("SELECT e.entrada_ref, e.salida_ref from efectivo e inner join factura_venta f using(f_nro) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and f.usuario='$user' and f.clase ='$clase'");
		while($link->NextRecord()){
        ...
        // Ventas contado
	private function getVentasContadoServicio($f_nro){
		$data = array();
		$link = new My();
		$monto = 0;
		$link->Query("SELECT e.entrada_ref, e.salida_ref from efectivo e inner join factura_venta f using(f_nro) where f.f_nro = $f_nro and f.estado = 'Cerrada'  ");
		while($link->NextRecord()){
			$monto += (float)$link->Record['entrada_ref'] - (float)$link->Record['salida_ref'];
		}
		return $monto;
        ...
        // Ventas contado
	private function getChequesServicio($f_nro){
		$data = array();
		$link = new My();
		$monto = 0;
		$link->Query("SELECT valor_ref from cheques_ter e inner join factura_venta f using(f_nro) where f.f_nro = $f_nro and f.estado = 'Cerrada'  ");
		while($link->NextRecord()){
			$monto += (float)$link->Record['valor_ref']  ;
		}
		return $monto;
		$link->Close();
	}
	...
	private function getVentasConvenio($user,$suc,$desde,$hasta,$clase){
		$data['Tarjeta Credito'] = 0;
		$data['Tarjeta Debito'] = 0;
		$data['Retencion'] = 0;
		$link = new My();
		$convenios = $this->getConvenios();
                $Qry = "SELECT c.cod_conv,c.monto,t.tipo FROM convenios c INNER JOIN factura_venta f USING(f_nro) INNER JOIN tarjetas t ON c.cod_conv = t.cod_tarjeta  where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and f.usuario = '$user' and f.clase ='$clase'";
		$link->Query($Qry);
	...
	private function getVentasCheque($user,$suc,$desde,$hasta,$clase){
		$data['Al Dia'] = 0;
		$data['Diferido'] = 0;
		$link = new My();
		$link->Query("SELECT if(c.fecha_emis=c.fecha_pago,'Al Dia','Diferido') as tipo,sum(c.valor_ref) as monto from cheques_ter c inner join factura_venta f using(f_nro) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and f.usuario = '$user' and f.clase ='$clase' group by tipo");
		while($link->NextRecord()){
			$tipo = $link->Record['tipo'];
			$data[$tipo] += (float)$link->Record['monto'];
		}
	...
	// Sumatoria de cuotas
	private function getVentasCuotas($user,$suc,$desde,$hasta,$clase){
		$data = 0;
		$link = new My();
		$link->Query("SELECT sum(c.monto_ref) as monto from cuotas c inner join factura_venta f using(f_nro) where f.suc='$suc' and f.fecha_cierre between '$desde' and '$hasta' and f.estado = 'Cerrada' and f.usuario = '$user' and f.clase ='$clase'");
		if($link->NextRecord()){
	...
	// Sumatoria de cuotas
	private function getVentasCuotasServicio($f_nro){
		$data = 0;
		$link = new My();
		$link->Query("SELECT sum(c.monto_ref) as monto from cuotas c inner join factura_venta f using(f_nro) where f.f_nro='$f_nro'  and f.estado = 'Cerrada'  ");
		if($link->NextRecord()){
			$data = $link->Record['monto'];
		}
		$link->Close();
	...
	private function getDevoluciones($user,$suc,$desde,$hasta){
		$data = 0;
		$link = new My();
		$link->Query("SELECT sum(n.total) as monto from nota_credito n inner join factura_venta f using(f_nro) where n.suc='$suc' and n.fecha between '$desde' and '$hasta' and f.estado = 'Cerrada' and n.estado in ('Cerrada','Reconciliada') and f.usuario = '$user'");
		if($link->NextRecord()){
			$data = $link->Record['monto'];
		}
		$link->Close();
	...
	private function getNotasCredito($suc,$desde,$hasta){
		$link = new My();
		$link->Query("SELECT n_nro, concat(u.nombre,', ',u.apellido) as vendedor, n.cliente, n.f_nro,f.moneda, n.total from nota_credito n inner join factura_venta f using(f_nro) inner join usuarios u on f.usuario=u.usuario where f.suc='$suc' and n.fecha between '$desde' and '$hasta' and f.estado = 'Cerrada' and (n.estado='Cerrada' or n.estado='Reconciliada') order by vendedor asc");
		$notasCredito = array();
		while($link->NextRecord()){
	...
	private function getCheques($suc,$desde,$hasta,$tipo){
		...
		if($tipo == "ventas"){
		   ...
		   $query = "SELECT trans_num as id_cobro,benef,b.nombre,c.cuenta,c.nro_cheque,c.valor_ref,DATE_FORMAT(c.fecha_emis,'%d-%m-%Y') as emis,DATE_FORMAT(c.fecha_pago,'%d-%m-%Y') as pago,tipo from cheques_ter c inner join bancos b using(id_banco) where c.id_concepto=8 and c.fecha_ins between '$desde' and '$hasta' and c.suc = '$suc' and estado <> 'Anulado'";
		}
	...
	private function getVentasCredito($suc,$desde,$hasta){
		$link = new My();
		$link->Query("SELECT f.f_nro, f.moneda,cliente,COUNT(c.f_nro) AS cuotas,SUM(c.monto) AS total,clase  FROM factura_venta f, cuotas c WHERE f.f_nro = c.f_nro AND f.suc = '$suc' AND f.fecha_cierre BETWEEN '$desde' AND '$hasta' and f.estado ='Cerrada' and f.clase = 'Articulo' GROUP BY f.f_nro,f.moneda, cod_cli ORDER BY cliente ASC, id_cuota ASC");
		$cuotas = array();
		while($link->NextRecord()){
		...
		$link = new My();
		$link->Query("SELECT p.id_pago as id_cobro, cliente,SUM(entrega_actual) AS valor,p.folio_num AS recibo FROM pagos_recibidos p, pago_rec_det d WHERE p.id_pago = d.id_pago AND suc = '$suc'  AND  fecha BETWEEN '$desde' AND '$hasta' GROUP BY cliente,p.folio_num");
		$cobros = array();
		while($link->NextRecord()){
		    array_push($cobros, $link->Record);
                ...
                $link->Query("SELECT concat('Pago Parcial Factura: ',f.f_nro) AS id_cobro,cliente,entrada_ref AS valor, f.nro_recibo AS recibo FROM factura_venta f, efectivo e WHERE f.f_nro = e.f_nro AND  nro_recibo IS NOT NULL  AND cant_cuotas > 0 AND f.suc = '$suc' AND f.fecha_cierre BETWEEN '$desde' AND '$hasta'");
	...
	private function getMonedasSum($suc,$desde,$hasta){
		...
		//$query = "SELECT replace(lcase(m.m_cod),'$','s') as moneda, sum(if((f.f_nro is not null and f.estado = 'Cerrada') or f.f_nro is null,e.entrada,0)) - sum(if((f.f_nro is not null and f.estado = 'Cerrada') or f.f_nro is null,e.salida,0)) as mov from efectivo e inner join factura_venta f on e.f_nro=f.f_nro inner join monedas m using(m_cod)  where e.suc='$suc' and e.fecha between '$desde' and '$hasta' group by m.m_cod union select 'sum', sum(if((f.f_nro is not null and f.estado = 'Cerrada') or f.f_nro is null,e.entrada_ref,0))-sum(if((f.f_nro is not null and f.estado = 'Cerrada') or f.f_nro is null,e.salida_ref,0)) as mov from efectivo e inner join factura_venta f on e.f_nro=f.f_nro inner join monedas m using(m_cod) where e.suc='$suc' and e.fecha between '$desde' and '$hasta'";	
                
                $query = "SELECT REPLACE(LCASE(e.m_cod),'$','s') AS moneda, SUM(entrada-salida) AS mov  FROM efectivo e WHERE suc = '$suc' AND fecha between '$desde' and '$hasta' GROUP BY m_cod UNION  SELECT 'sum',SUM(entrada_ref-salida_ref) AS mov FROM efectivo e WHERE suc = '$suc' AND fecha between '$desde' and '$hasta' ";
