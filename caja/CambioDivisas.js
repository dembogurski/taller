    ...
    $.post("caja/CambioDivisas.class.php", { "action": "getMovimientos", "args": suc + ',' + fecha }, function(data) {
        if (data.err) {
            ...
            $.each(datos, function(i, val) {
                $("<option/>", {
                    "html": "<img heigth='16' width='16' src='/marijoa_sap/img/" + (datos[i].moneda).toLowerCase().replace("$", "s") + ".png'/>" + datos[i].moneda
                }).appendTo("#com");
            });
            if(data.cotiz){
...
function cotizXSuc() {
    ...
    if (currentCotiz.length === 0) {
        ...
        for (var i in currentCotiz) {
            $("<option/>", {
                "value": i,
                "html": "<img heigth='16' width='16' src='/marijoa_sap/img/" + (i).toLowerCase().replace("$", "s") + ".png'/>" + currentCotiz[i].m_cod
            }).appendTo("#ch");
        }
    ...
    if (monedaEntrada !== null && monedaSalida !== null) {
        ...
        if (entrada_lock === false || salida_lock === false) {
            if (entrada_lock === false) {
                ...
                entrada = cambio;
                $("#cant_entrada").val(entrada.toFixed(6));
            } else if (salida_lock === false) {
                var cotiz = parseFloat(eval("currentCotiz." + monedaEntrada + ".venta"));
                salida = parseFloat($("#cant_entrada").val() * cotiz);
                $("#cant_salida").val(salida.toFixed(6));
            }
        }
        // Limpiar
        $(".operaciones").empty();
        ...
        if (parseFloat(entrada).format(6,3,'.',',') != cambio.format(6,3,'.',',')) {
            ...
            var cotiz_salida = eval("currentCotiz." + monedaSalida + ".compra");
            //diferencia = -eval((salida * cotiz_salida) + '-(' + entrada + '*' + cotiz_ent + ')');
            diferencia = eval((salida * cotiz_salida) + '-(' + entrada + '*' + cotiz_ent + ')');
            $("<p/>", {
                "class": "entrada_calc",
                "text": "Entrada Calculada: " + cambio.format(6,3,'.',',') + ' ' + monedaEntrada
            }).appendTo("#operaciones");

            $("<p/>", {
                "class": "entrada_calc",
                "text": "Diferencia: " + diferencia.format(6,3,'.',',') + 'G$'
            }).appendTo("#operaciones");
        }
        /*
        $("<p/>", {
...
function process() {
    var dateCheck = validDate($("#cambioFecha").val());
    var args = [getSuc(), dateCheck.fecha, $("#com").val(), $("#cant_salida").val(), $("#ch").val(), $("#cant_entrada").val()];
