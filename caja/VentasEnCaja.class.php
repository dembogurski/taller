...
class VentasEnCaja {
    ...
    function main() {
        ...
        $db->Query("SELECT f.f_nro as nro, DATE_FORMAT(f.fecha,'%d-%m-%Y') AS fecha, f.cod_cli ,f.ruc_cli AS ruc,f.tipo_doc_cli, cliente, cat ,   total ,  total_desc,cod_desc,   total_bruto,desc_sedeco, empaque,pref_pago, usuario as vendedor, f.estado,moneda FROM factura_venta f  WHERE f.estado = 'En_caja' AND suc = '$suc'  ");
...
function buscarTicket(){
    ...
    $suc = $_POST['suc'];
    $db = new My();
    $Qry = "SELECT cod_cli,ruc_cli,cliente,cat,cod_desc,DATE_FORMAT(fecha_cierre,'%d-%m-%Y') AS fecha,moneda, total,total_desc ,fact_nro FROM factura_venta WHERE f_nro = $ticket AND estado = 'Cerrada' AND suc ='$suc'";
    $db->Query($Qry);
    $array = array();
    if($db->NumRows()>0){
        $db->NextRecord();
