...
class Pagare {
   ...
   function main(){
        ...
        $Q0 = new My();
        $query0 = "SELECT f_nro AS REF,id_cuota AS NRO, valor_total AS MONTO, moneda AS MONEDA,m_descri as NOMBRE_MONEDA, DATE_FORMAT(vencimiento,'%d') AS DIA_VENC, DATE_FORMAT(vencimiento,'%m') AS MES_VENC,DATE_FORMAT(vencimiento,'%Y') AS ANIO_VENC "
        ...
        $DENOMINACION = '<B>[CAMBIAR LA DENOMINACION DE LA EMPRESA]</B>';
        ...
        $ms->Query("SELECT cod_cli AS  CardCode, nombre AS cli_fullname, tel AS cli_tel1,ciudad,dir FROM clientes WHERE ci_ruc = '$CI' ");
        $ms->NextRecord();
        $COD_CLI = $ms->Record['CardCode'];
        $CLIENTE =  ($ms->Record['cli_fullname']);
        $TEL = $ms->Record['cli_tel1'];
