...
class CambioDivisas {
    ...
    public function process($suc,$date,$curr_out,$curr_out_amount,$curr_in,$curr_in_amount,$ajuste=false){
        $my_link = new My();
		$cotiz = 1;
		$calc_cotiz = 0;
		$dif = 0;
        ...
        $curr_in_check = ($curr_in=="G$")?($curr_out_amount*$curr_out_ref_cotiz):$curr_in_amount;

        $query_out = "INSERT into efectivo (id_concepto,m_cod,salida,cotiz,salida_ref,fecha_reg,fecha,hora,suc,estado) values (6,'$curr_out',$curr_out_amount,$curr_out_ref_cotiz,($curr_out_amount*$curr_out_ref_cotiz),current_date,'$date',current_time,'$suc','Pendiente')";
        ...
        $query_in = "INSERT into efectivo (id_concepto,m_cod,entrada,cotiz,entrada_ref,fecha_reg,fecha,hora,suc,estado) values (5,'$curr_in',$curr_in_check,$curr_in_ref_cotiz,($curr_out_amount*$curr_out_ref_cotiz),current_date,'$date',current_time,'$suc','Pendiente')";
		...
		//Diferencia en la cotizacion
        if((int)$calc_cotiz != (int)$cotiz){
            $val = '';
			$msg .= ', c-'.(int)$cotiz;
			//Ajustar
            //$my_link->Query("INSERT into efectivo (id_concepto,m_cod,salida,cotiz,salida_ref,fecha,hora,suc,estado) values (9,$curr_out,$curr_out_ref_cotiz,$curr_out_ref_cotiz*$curr_out_amount,current_date,current_time,$suc,'Pendiente')");
        }
		//Deferencia por decimal
		if($dif>=1 || $dif<=-1 || $ajuste!=false){			
			$diferencia = (!$ajuste)?$dif:$ajuste;
			$msg .= ', d: '.round((float)$diferencia,2);
			//$id_concepto = ($diferencia>=1)?10:11;
			//$tipe_cotiz = ($diferencia>=1)?"entrada,entrada_ref":"salida,salida_ref";
			//$cantidad = ($diferencia<0)?'-'.$diferencia:$diferencia;
			$id_concepto = ($diferencia>=1)?10:11;// 10:positivo, 11:negatico
			$tipe_cotiz = ($diferencia<=1)?"salida,salida_ref":"entrada,entrada_ref";
			$cantidad = ($diferencia<0)?'-'.$diferencia:$diferencia;
			// Ajustar
			$my_link->Query("INSERT INTO efectivo (id_concepto,m_cod,cotiz,$tipe_cotiz,fecha_reg,fecha,hora,suc,estado) VALUES ($id_concepto,'G$',1,$cantidad,$cantidad,current_date,'$date',current_time,'$suc','Pendiente')");
		}
		echo $msg.'"}';
