...
class StockYMovimientoArticulos {
   function __construct() {
    ...
    function main() {
        ...
        $ms = new MS();
        $ms->Query("SELECT ItemCode,ItemName FROM OITM  WHERE ItmsGrpCod like '$sector' AND ItemCode like '$codigo';");    
        $tmp_cod ="";
        while($ms->NextRecord()){
...
function getStock(){
    ...
    $ms->Query(" select sum(Quantity) as Cant from oibt where ItemCode = '$codigo' and WhsCode = '$suc' and Quantity > 0  and U_fin_pieza <> 'Si'");
    
    if($ms->NumRows()>0){
        $ms->NextRecord();
        $cant_suc = $ms->Record['Cant'];
        if($cant_suc == null){
            $cant_suc = 0;
        }
    }
    ...
    // Global
    $ms->Query(" select sum(Quantity) as Cant from oibt where ItemCode = '$codigo'  and Quantity > 0  and U_fin_pieza <> 'Si'");
    $cant  = 0;
    if($ms->NumRows()>0){
        $ms->NextRecord();        
        $cant = $ms->Record['Cant'];
...
function drawChart(){
   ...
   if($tipo == "cliente"){
       ...
       $ms = new MS();
       $ms->Query("SELECT Phone1,CardName FROM OCRD WHERE CardCode = '$CardCode'");
       $ms->NextRecord();
