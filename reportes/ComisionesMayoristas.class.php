...
function getPagosRecibidos($factura){
   $ms = new MS();
   $sql = "SELECT i.DocTotal,sum(SumApplied) as SumApplied  FROM ORCT o, RCT2 r, OINV i WHERE r.DocEntry = i.DocEntry  and o.DocNum = r.DocNum AND  o.Canceled != 'Y' and i.U_Nro_Interno = $factura group by i.DocTotal,i.U_Nro_Interno";    
   $ms->Query($sql);
   ...
   if($ms->NumRows()>0){
       ...
       $SumApplied = $ms->Record['SumApplied'];
       $SumApplied += 0+ $nota_credito;
       return array("TotalFactura"=>$DocTotal,"Cobrado"=>$SumApplied);
   }else{
       $sql = "SELECT i.DocTotal   FROM  OINV i WHERE  i.U_Nro_Interno = $factura";    
       $ms->Query($sql);
       if($ms->NumRows()>0){
         $ms->NextRecord();
         $DocTotal = $ms->Record['DocTotal'];
         $SumApplied = $nota_credito;
         return array("TotalFactura"=>$DocTotal,"Cobrado"=>$SumApplied); 
       }else{
          echo "<b style='color:red'>Error en Factura: $factura, comunicar a informatica </b> $sql";
          die(); 
       }   
