...
class CalculoVariable {
    ...
    function getVentas($desde_eng, $hasta_eng, $usuario,$tipo) {
        /*
          $ms = new MS();
          $sqlserver = "SELECT U_CAT as CAT, sum(case when U_estado_venta is null OR U_estado_venta = 'Normal' then LineTotal else 0 end) 'Normal',  sum(case when U_estado_venta is not null and U_estado_venta != 'Normal' then LineTotal else 0 end) 'Oferta'
          FROM  OINV o, INV1 i WHERE o.DocEntry = i.DocEntry and U_vendedor = '$usuario' and o.DocDate between '$desde' and '$hasta'  GROUP BY  U_CAT";
          $ms->Query($sqlserver);
         * */
