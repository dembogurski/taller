...
class VentasXVendedor {
    ...
    function agrupado($desde,$hasta,$suc,$usuario,$t,$categorias){
        $db = new My();
        $t->Show("head_agrup");      
        $db->Query("SELECT f.suc, f.usuario,SUM(d.cantidad) as cant,SUM( IF(d.estado_venta = 'Normal', d.subtotal,0)) AS Normal,SUM( IF( d.estado_venta <> 'Normal', d.subtotal,0)) AS Oferta,SUM(d.subtotal) AS Total, SUM( IF(d.estado_venta = 'Normal', descuento,0)) AS DescNormal,SUM( IF(d.estado_venta <> 'Normal', descuento,0)) AS DescOferta,SUM(descuento) AS Descuento, SUM(nd.subtotal)  AS Devolucion, SUM(d.subtotal) AS Neto FROM factura_venta f INNER JOIN  fact_vent_det d ON f.f_nro = d.f_nro LEFT JOIN nota_credito n ON f.f_nro = n.f_nro LEFT JOIN nota_credito_det nd ON n.n_nro = nd.n_nro AND   d.lote = nd.lote WHERE f.estado = 'Cerrada' AND f.fecha_cierre BETWEEN '$desde' AND '$hasta'  AND f.usuario LIKE '$usuario' AND f.suc LIKE '$suc' and f.cat in ($categorias) GROUP BY f.suc,f.usuario ORDER BY f.suc,f.usuario");
