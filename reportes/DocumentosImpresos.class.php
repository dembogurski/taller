...
class DocumentosImpresos {
    ...
    function __construct() {       
        ...
        $filename = 'StarSoft_'.str_replace('/','-',$desde_r).'_'.str_replace('/','-',$hasta_r).'.xlsx';

        $this->template->Show("head");
        //$this->template->Show("table_header");
        $this->facturaContado();
        $this->facturaCredito();
        $this->notasCredito();
        //$this->template->Show("table_footer");        
        ...
        if(count($this->toExcelFile)>0){
            ini_set('memory_limit', '2048M');
            $this->excel->getActiveSheet()->fromArray(array_keys(current($this->toExcelFile)), null, 'A1');
            $this->excel->getActiveSheet()->fromArray($this->toExcelFile, null, 'A2');/*
            $cacheMethod = PHPExcel_CachedObjectStorageFactory:: cache_to_phpTemp;
            $cacheSettings = array( ' memoryCacheSize ' => '512MB');
            PHPExcel_Settings::setCacheStorageMethod($cacheMethod, $cacheSettings);*/
            $writer = PHPExcel_IOFactory::createWriter($this->excel, 'Excel2007');

            $writer->save("startsoft/$filename");
            $this->template->Set("archivo",$filename);
            $this->template->Set("descarga","startsoft/$filename");
    ...
    private function facturaContado(){
        ...
        while($link->NextRecord()){
            ...
            foreach($link->Record as $key=>$value){                
                $this->template->Set($key,(String)$value);
            }
            $this->template->Show("table_body");*/
        }
