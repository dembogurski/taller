...
class VentasPorSectorArticulos{
   ...
   function __construct(){
      ...
      if($this->cliente !== '%'){
            $msc = new MS();
            $msc->Query("SELECT CardName FROM OCRD WHERE LicTradNum = '$this->cliente' AND CardType = 'C'");
            if($msc->NextRecord()){
                $cliNombre = $msc->Record['CardName'];
            }
      }
   ...
   function getArticulosInGrupo($grupo){
      ...
      $link->Query("SELECT ItmsGrpCod,RTRIM(SUBSTRING(ItemName,1,CHARINDEX('-',ItemName)-1)) as grupo,ItemCode,LTRIM(SUBSTRING(ItemName,CHARINDEX('-',ItemName)+1,LEN(ItemName))) as ArticuloNombre FROM OITM where ItmsGrpCod like '$grupo' order by ArticuloNombre asc");
      while($link->NextRecord()){
         $articulos[$link->Record['ItemCode']] = $link->Record['ArticuloNombre'];
         $grupos[$link->Record['ItemCode']] = $link->Record['grupo']; 
      }
