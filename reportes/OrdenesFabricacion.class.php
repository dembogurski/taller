...
class OrdenesFabricacion  {
    ...
    function main() {
        ...
        $Qry = "SELECT o.nro_orden,sap_doc,cliente,usuario,DATE_FORMAT(fecha,'%d-%m-%Y') AS fecha,suc,estado,codigo,descrip,design,cantidad,largo AS medida,o.obs    
...
function getEstadoOrdenSAP(){
    require_once("../Y_DB_MSSQL.class.php"); 
    $sap_doc = $_REQUEST['sap_doc'];
    $ms = new MS();
    $ms->Query("SELECT Status FROM OWOR WHERE DocNum = $sap_doc;");
    $ms->NextRecord();
...
function getEstado(){
    ...
    if($estado == "En Proceso"){
        ...
        while($db->NextRecord()){
            ...
            $codigo = $db->Record['codigo'];
            $lote = $db->Record['lote'];
            $descrip = getDescripProductoTerminado($codigo);
