...
function appendDetail(codigo,lote,descrip,cant,um,precio,subtotal,total,senha){
    ...
    $("#detalle_reserva").append('<tr id="tr_'+lote+'"><td class="item codigo_lote">'+lote+'</td><td class="item">'+descrip+'</td><td class="num cantidad">'+cant+'</td><td  class="itemc">'+um+'</td><td class="num">'+precio+'</td> <td class="num">'+subtotal+'</td><td class="itemc"><img class="del_det" title="Borrar Esta Pieza" style="cursor:pointer" src="img/trash_mini.png" onclick=delDet("'+codigo+'","'+lote+'")></td></tr>');
    $("#detalle_reserva").append('<tr class="tr_total_reserva" style="font-weight: bolder"><td >&nbsp;Totales</td><td colspan="4" id="msg_det" style="text-align: center;font-size: 11" class="info"></td> <td style="text-align: right;" id="total_reserva" class="num">'+total+'&nbsp;Gs.</td><td></td> </tr>');
    $("#detalle_reserva").append('<tr class="tr_total_reserva" style="font-weight: bolder"><td >&nbsp;Se&ntilde;a</td><td colspan="4" id="msg_det" style="text-align: center;font-size: 11" class="info"></td> <td style="text-align: right;" id="total_senha" class="num">'+senha+'&nbsp;Gs.</td><td></td> </tr>');
    $("#finalizar").removeAttr("disabled");
}
    ...
    $.ajax({
        ...
        success: function(data){ 
            ...
            if( mensaje === "Ok" ){
                ...
                var precio = parseFloat(Math.round(data[0].Precio - (data[0].Precio * data[0].Descuento) / 100));//parseFloat(  data[0].Precio ).format(0, 3, '.', ',');
                $("#um").val(data[0].UM);  
                $("#precio_cat").val(precio);
                $("#precio_venta").val(precio);
                $("#stock").attr("title","Stock Global: "+data[0].TotalGlobal+" "+data[0].UM);
   ...
   //window.open("ventas/Ticket.class.php?factura="+factura+"&cliente="+cliente+"&ruc="+ruc+"&suc="+suc+"","Ticket de Venta","width=400,height=560,scrollbars=yes"); 
    ...
    $.ajax({
        ...
        complete: function(objeto, exito) {
            if (exito == "success") {
                 if(objeto.Ok){
                    $("#msg_codigo").html("La Reserva ha sido enviada a caja..."); 
                    $("#alert_msg").html("Impresion de Ticket en Progreso...<br>");
