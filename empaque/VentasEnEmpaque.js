...
function setHotKeysDetalle() {
    ...
    $(".falla").click(function() {
        ...
        var um_prod = $(".um_prod_" + fila).html();
        var codigo_falla = $(".cm_falla_" + fila).attr("data-cod_falla");

        if (um_prod != "Unid" && codigo_falla != "") {
            var actual = $(this).html();
            var position = $(this).position();
            var w = $(this).width()
    ...
    if (!guardando || ignorarControles) {
        ...
        if (medicion != "0" && ajuste != "") {
            ...
            $.ajax({
                type: "POST",
                url: "Ajax.class.php",
                data: { "action": "guardarControlEmpaque", factura: factura, lote: lote, medicion: medicion, sis_med: sis_med, calc: calc, diff: diff, fuera_rango: fr, ajuste: ajuste, verificador: getNick(), falla: falla, um: um },
                async: true,
                dataType: "html",
                beforeSend: function() {
...
function calcularMedicion(fila) {
    ...
    var sis_med = $(".sis_" + fila).val();
    var um_prod = $(".um_prod_" + fila).text();
    var falla = parseFloat($(".cm_falla_" + fila).html());
    ...
    if (medido > 0) {
        console.log("Medido " + medido + "  UM Prod: " + um_prod + "  sis_med " + sis_med);
        if (um_prod === "Mts") {
            var ct = cantidad + falla;

            var vt = parseInt(ct + 1) * tolerancia;
            ...
            if (sis_med === "Kg") {
                long_calc = ((medido * 1000) / (gramaje * ancho)).toFixed(2);
        ...
        if (um_prod === "Kg") {
            ...
            var total = cantidad + kilaje_falla;
            var metraje_total_cargado = redondear((total * 1000) / (gramaje * ancho));
            ...
            if (sis_med === "Kg") {
                var metraje_total_medido = redondear((medido * 1000) / (gramaje * ancho));
                diff = redondear(metraje_total_medido - metraje_total_cargado);
                //console.log("Rango Bajo: "+rb+"    Correcto: "+metraje_total_cargado+"    Rango Alto: "+ra+"   "+metraje_total_medido);
        ...
        success: function(data) {
            ...
            if (data.Error != "Factura en Edicion") {
                ...
                for (var i in data) {
                    ...
                    //console.log(um +" sis_med "+sis_med);
                    var show_balanza = "inline";

                    if ((um == "Mts" && cantidad >= 5)) {
                        cant_calc = parseFloat(cantidad) + parseFloat(cant_falla);

                        show_balanza = "none";
                        kg_med = parseFloat(cantidad) + parseFloat(cant_falla);;
                        sis_med = "Mts";
                        //console.log("Lote: "+lote+"  um: "+um+" Cant Medida: "+cant_med+" kg_calc: "+kg_calc);

                    } else if ((um == "Mts") && (cantidad < 5)) {
                        if (sis_med == "Kg") {
                            //kg_med = kg_calc; // Si es Kg 
                    ...
                    if (um == "Kg") {
                        var longitud_calc = (kg_calc * 1000) / (gramaje * ancho);
                        kg_med = kg_calc;
                        cant_calc = cantidad;
                    }
                    if (um == "Unid") {
                    ...
                    if (gramaje < 0) {
                        ...
                        if (um == "Unid") {
                            sis_med = "Unid";
                        } else { // Esta mal entonces medir en metros
                            sis_med = "Mts";
                        }
                    }
                    if (!pre_control && !(um == "Mts" && cantidad >= 5)) { // Ya guardo antes
                        kg_med = "";
                    }
                    ...
                    $("#detalle_factura").append("<tr class='det_codigo fila_" + j + "' id=tr_" + lote + " data-fila='" + j + "' data-margen='5' data-stock_real='"+stock_actual+"' data-img='"+img+"' ><td class='item select'><span class='lote' data-precio='" + precio_venta + "'>" + lote + "</span></td><td class='item select'><span class='descrip' data-codigo='"+codigo+"'>" + descrip + "</span> </td><td class='num cantidad cant_" + j + "'>" + cantidad + "</td>  \n\
                <td class='itemc um_prod_" + j + "'>" + um + "</td><td class='falla itemc " + cod_falla + " cm_falla_" + j + "' data-cod_falla='" + cod_falla_e + "'  >" + cant_falla + "</td>\n\
