...
function facturar_vendedor(){
    ...
    /*
    $queryOut = "UPDATE fact_vent_det SET f_nro = '$ticketEntrada', cant=$cantEntrada WHERE f_nro='$ticketSalida' and lote = '$loteSalida'";   

    if((bool)$datos['eliminar']){

        echo "Eliminar \n";
    }else{
        $my_link = new My();
        $ms_link = new MS();
        $datos_ticket = "SELECT usuario,tipo_doc_cli as tipo_doc,cat FROM factura_venta WHERE f_nro=$ticketSalida";
        $datos_lote = "SELECT o.DistNumber as lote,o.ItemCode as codigo,LTRIM(RTRIM(SUBSTRING(m.ItemName,CHARINDEX ('-',m.ItemName)+1,LEN(m.ItemName))))+'-'+c.Name as descrip, InvntryUom as um,U_tara as tara,U_gramaje as gramaje,o.U_ancho as ancho,cast(round(q.Quantity - ISNULL(q.CommitQty,0),2) as numeric(20,2)) as Stock,o.U_estado_venta as estado_venta FROM OBTN o inner join OBTW w on o.SysNumber=w.SysNumber and o.ItemCode=w.ItemCode inner join OBTQ q on o.SysNumber=q.SysNumber and w.WhsCode=q.WhsCode and q.ItemCode=w.ItemCode inner join OITM m on o.ItemCode=m.ItemCode LEFT JOIN [@EXX_COLOR_COMERCIAL] c ON o.U_color_comercial = c.Code 
        where w.WhsCode = '00' AND  o.DistNumber in ($loteEntrada)";

        $my_link->Query($datos_ticket);
        $my_link->NextRecord();
        foreach($my_link->Record as $key=>$value){
            $datos_insert[$key]=$value;
        }

        $ms_link->Query($datos_lote);
        $ms_link->NextRecord();
        foreach($ms_link->Record as $key=>$value){
            $datos_insert[$key]=$value;
        }
        
        $datos_insert['um_prod']= $datos_insert['um'];
        echo "No Eliminar \n";
    }
