...
function controlarCotizacion(){
    ...
    if(moneda != "G$"){
    var fecha = validDate($("#fecha").val()).fecha;
    $.post( "Ajax.class.php",{ action: "controlarCotizacionEntradaMercaderias",moneda:moneda,fecha:fecha}, function( data ){            
        ...
        success: function(data) { 
            ...
            if(data.length > 0){
                ...
                for(var i in data){        
                    ...
                    var Gramaje = parseFloat( (data[i].U_GRAMAJE)).format(0, 3, '.', ',');
