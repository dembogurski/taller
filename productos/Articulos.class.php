...
class Articulos {
    ...
    private $items = [
        ...
        array("column_name" => "cod_sector", "nullable" => "NO", "data_type" => "int", "max_length" => "", "numeric_pres" => "10", "dec" => "0", "titulo_campo" => "Cod Sector=>", "titulo_listado" => "Cod Sector", "type" => "db_select", "required" => "", "inline" => "false", "editable" => "Yes", "insert" => "Yes", "default" => "sectores=>cod_sector,descrip", "pk" => "MUL", "extra" => ""), array("column_name" => "um", "nullable" => "NO", "data_type" => "varchar", "max_length" => "10", "numeric_pres" => "", "dec" => "", "titulo_campo" => "Um=>", "titulo_listado" => "Um", "type" => "db_select", "required" => "", "inline" => "false", "editable" => "readonly", "insert" => "Yes", "default" => "unidades_medida=>um_cod,um_descri", "pk" => "MUL", "extra" => ""), 
    ...
    function desasignar(){
        ...
        $ids = json_decode($_REQUEST['ids']);         
        foreach ($ids as $cod_uso) {
            $db->Query("delete from  marijoa.art_x_uso where codigo = '$codigo' and cod_uso = $cod_uso;"); 
        }
        echo json_encode(array("mensaje"=>"Ok"));
    }
    ...
    function generarCodigo(){
        ...
        $sql = "SELECT CONCAT(prefijo, LPAD(serie,longitud,'0')) AS codigo FROM sectores WHERE cod_sector = $cod_sector";  
        echo json_encode($this->getResultArray($sql));
    }
    function getProximoCodigo($cod_sector){
       $sql = "SELECT CONCAT(prefijo, LPAD(serie,longitud,'0')) AS codigo, serie FROM sectores WHERE cod_sector = $cod_sector";   
    ...
    function addToListaMat(){
         ...
         if($cant > 0){
             ...
             $ins = " INSERT INTO marijoa.lista_materiales(codigo, articulo, ref, descrip, um, cantidad, rendimiento, precio_unit, sub_total)
