            ...
            success: function (data) {  
                 ...
                 if(data.length > 0){
                     for(var i in data){
                        ...
                        $("#stock_x_suc").append("<tr class='row_inv'><td  class='itemc'>"+suc+"</td><td class='item'>"+nombre+"</td><td  class='num'>"+stock+"</td><td  class='itemc'>"+estado_venta+"</td></tr>");
                     }
                     $("#msg_articulos").html("");   
...
function cambiarPrecio(){
    ...
    $(".precio_venta").each(function () {
        ...
        if(regla_redondeo == "SEDECO"){
            calc = parseFloat(redondeo50(calc)).format(0, 3, '.', ',') ;  
