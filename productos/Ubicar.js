...
function moverPallet(pallet,suc,tipo,nombre,fila,col){
    $.ajax({
        type: "POST",
        url: "productos/Ubicar.class.php",
        data: {"action": "reubicarPallet",pallet:pallet, suc: suc, tipo:tipo,nombre:nombre,fila:fila,col:col},
        async: true,
        dataType: "html",
        beforeSend: function () {
...
function buscarCodigoEnDocumentos(lote, codigo, ubicacion, tipo, fila, col, pallet, descrip,obs,stock){
    ...
    $.ajax({
            type: "POST",
            url: "productos/Ubicar.class.php",
            data: {"action": "buscarCodigoEnDocumentos", codigo: codigo,lote:lote,suc:getSuc()},
            async: true,
            dataType: "json",
            beforeSend: function () { 
            ...
            success: function (data) {   
                ...
                if(mensaje=='Ok' ){   
                   ...
                   var td = data.TipoDocumento;
                   var nro = data.Nro;
                   var fecha = data.fecha;
                   var suc = data.suc;
                   var estado = data.estado;
                   var cantidad = data.cantidad; 
                   $("#msg_codigo").html("Articulo en Documento: "+td+"  "+nro+"  Fecha"+fecha+"  Suc:"+suc+"  Estado: "+estado+"  Cantidad: "+cantidad+"");  
                }
            }
    });     
}
