...
class CargarNotaRemision {
    ...
    function __construct() {
        ...
        if (isset($_POST['action'])) {
            ...
            while ($db->NextRecord()) {
                ...
                $datosLote = $this->getDatosLotes($codigo, $lote, $suc_origen);
                $UM = $datosLote['UM'];
    ...
    function getDatosLotes($codigo, $lote, $suc) {
        ...
        if($mnj_x_lotes == "Si"){
            $ms->Query("SELECT tara AS U_tara,gramaje AS U_gramaje,l.ancho AS U_ancho,padre AS U_padre,s.kg_ent AS U_kg_desc, s.cantidad AS Quantity FROM stock s, articulos a , lotes l 
            ...
            $ms->Query("SELECT 0 AS U_tara,0 AS U_gramaje,a.ancho AS U_ancho,'' AS U_padre,s.kg_ent AS U_kg_desc, SUM(s.cantidad) AS Quantity  FROM stock s, articulos a 
